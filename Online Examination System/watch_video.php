...
if(isset($_POST['like_content'])){
   ...
   if($user_id != ''){
      ...
      $teacher_id = $fetch_content['teacher_id'];
      ...
      if($select_likes->rowCount() > 0){
         ...
         $insert_likes = $conn->prepare("INSERT INTO `likes`(user_id, teacher_id, content_id) VALUES(?,?,?)");
         $insert_likes->execute([$user_id, $teacher_id, $content_id]);
...
if(isset($_POST['add_comment'])){
   ...
   if($user_id != ''){
      ...
      if($select_content->rowCount() > 0){
         ...
         $select_comment = $conn->prepare("SELECT * FROM `comments` WHERE content_id = ? AND user_id = ? AND teacher_id = ? AND comment = ?");
         $select_comment->execute([$content_id, $user_id, $teacher_id, $comment_box]);

         if($select_comment->rowCount() > 0){
            $message[] = 'comment already added!';
         }else{
            $insert_comment = $conn->prepare("INSERT INTO `comments`(id, content_id, user_id, teacher_id, comment) VALUES(?,?,?,?,?)");
            $insert_comment->execute([$id, $content_id, $user_id, $teacher_id, $comment_box]);
...
<section class="watch-video">
   ...
   <?php
      ...
      if($select_content->rowCount() > 0){
         while($fetch_content = $select_content->fetch(PDO::FETCH_ASSOC)){
            ...
            $select_teacher->execute([$fetch_content['teacher_id']]);
   ...
   <div class="video-details">
      ...
      <div class="teacher">
         <img src="uploaded_files/<?= $fetch_teacher['image']; ?>" alt="" height="150" width="150">
         <div>
...
<section class="comments">
   ...
   <div class="show-comments">
      ...
      <div class="box" style="<?php if($fetch_comment['user_id'] == $user_id){echo 'order:-1;';} ?>">
         <div class="user">
            <img src="uploaded_files/<?= $fetch_commentor['image']; ?>" alt=""width="150" hight="150">
            <div>

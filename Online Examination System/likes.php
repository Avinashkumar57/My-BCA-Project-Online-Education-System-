      ...
      if($select_likes->rowCount() > 0){
         while($fetch_likes = $select_likes->fetch(PDO::FETCH_ASSOC)){
            ...
            if($select_contents->rowCount() > 0){
               ...
               $select_teachers = $conn->prepare("SELECT * FROM `teachers` WHERE id = ?");
               $select_teachers->execute([$fetch_contents['teacher_id']]);
               $fetch_teacher = $select_teachers->fetch(PDO::FETCH_ASSOC);
   ?>
   <div class="box">
      <div class="teacher">

...
if(isset($_POST['submit'])){
   ...
   if(!empty($profession)){
      ...
      $update_profession->execute([$profession, $teacher_id]);
      $message[] = 'profession updated successfully!';
   }

   if(!empty($email)){
      $select_email = $conn->prepare("SELECT email FROM `teachers` WHERE id = ? AND email = ?");
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `teachers` SET password = ? WHERE id = ?");
